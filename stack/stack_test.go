...
func TestStack(t *testing.T) {
	...
	if st.items[0] != (item{state: row3, node: nVal, value: "1"}) {
	...
	if st.items[0] != (item{state: row4, node: nSum, value: "1"}) {
	...
	if st.items[2] != (item{state: row7, node: nVal, value: "0"}) {
	...
	if st.items[0] != (item{state: row4, node: nSum, value: "(1 + 0)"}) {
	...
	if st.items[2] != (item{state: row8, node: nVal, value: "1"}) {
	...
	if st.items[0] != (item{state: row4, node: nSum, value: "((1 + 0) - 1)"}) {
...
func testCalc3(a, op, b string) (any, error) {
	return "(" + a + " " + op + " " + b + ")", nil
}
