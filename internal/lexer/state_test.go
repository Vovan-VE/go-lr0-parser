...
const srcStr = "Lorem ipsum dolor sit amet, consectepture"
const length = len(srcStr)

var source = []byte(srcStr)
...
func TestState_Len(t *testing.T) {
	...
	if b := a.to(15); b.Len() != length {
		t.Errorf("b.Len() is %v", b.Len())
	}
	if c := a.to(length); c.Len() != length {
...
func TestState_Offset(t *testing.T) {
	...
	if c := a.to(length + 3); c.Offset() != length {
...
func TestState_RestLen(t *testing.T) {
	...
	if b := a.to(15); b.RestLen() != length-15 {
		t.Errorf("b.RestLen() is %v", b.RestLen())
	}
	if c := a.to(length); c.RestLen() != 0 {
...
func TestState_RestBytes(t *testing.T) {
	a := NewState(source)
	if string(a.RestBytes()) != srcStr {
		t.Errorf("a.RestLen() is %q", a.RestBytes())
	}
	if b := a.to(15); string(b.RestBytes()) != srcStr[15:] {
		t.Errorf("b.RestLen() is %q", b.RestBytes())
	}
	if c := a.to(length); string(c.RestBytes()) != "" {
...
func TestState_BytesToOffset(t *testing.T) {
	...
	b := a.to(10)
	c := a.to(27)
	if s := a.BytesToOffset(15); string(s) != srcStr[:15] {
		t.Errorf("[1] is %q", s)
	}
	if s := b.BytesToOffset(31); string(s) != srcStr[10:31] {
		t.Errorf("[2] is %q", s)
	}
	if s := c.BytesToOffset(length + 5); string(s) != srcStr[27:] {
		t.Errorf("[3] is %q", s)
	}
...
func TestState_TakeByte(t *testing.T) {
	...
	c := a
	for rest := source[:]; len(rest) != 0; rest = rest[1:] {
		d, b := c.TakeByte()
		if b != rest[0] {
